...
def check_path(final_dir):
    if not os.path.exists(final_dir):
        ...
        if not os.path.isdir(final_dir):
            os.remove(final_dir)
            return check_path(final_dir)
        else:
            return False


def replace_local2remote(filen):
...
def replace_cn(fn):
    gp = GetAllPyFiles(fn_dir=fn, rped_name='../en', gogal_name='./en')
    
    # filen = os.path.join(os.getcwd(), 'Alkemieth')
    # gp = GetAllPyFiles(fn_dir=filen, rped_name='Alkemieth3', gogal_name='Alkemieth')
    
    gp.auto_run(nopy=False, valid=True, picture=False, mv_olddir_to_new_dir=False)
    gp.log.info("Replace cn en to en *.html done!")
    
    gp = GetAllPyFiles(fn_dir=fn, rped_name='../zh_CN', gogal_name='.')
    gp.auto_run(nopy=False, valid=True, picture=False, mv_olddir_to_new_dir=False)
    gp.log.info("Replace ../zh_CN to . in *.html done!")


def replace_en(fn):
    gp = GetAllPyFiles(fn_dir=fn, rped_name='../en', gogal_name='.')
    
    # filen = os.path.join(os.getcwd(), 'Alkemieth')
    # gp = GetAllPyFiles(fn_dir=filen, rped_name='Alkemieth3', gogal_name='Alkemieth')
    
    gp.auto_run(nopy=False, valid=True, picture=False, mv_olddir_to_new_dir=False)
    gp.log.info("Replace en to . in *.html done!")
...
if __name__ == '__main__':
    ...
    cn_path = os.path.join(dd, 'docs')
    en_path = os.path.join(cn_path, 'en')

    if check_path(cn_path):
        
        shutil.copytree(os.path.join(dd, 'source', 'zh_CN', 'build', 'html'), cn_path)
        with open(os.path.join(cn_path, 'CNAME'), 'w') as f:
            f.write('alkemine.org\n')
        replace_local2remote(cn_path)
        for i in ['_static', '_images', '_sources', '_downloads']:
            _rename_htmlconf(cn_path, i)
        replace_cn(cn_path)
        
        shutil.copytree(os.path.join(dd, 'source', 'en', 'build', 'html'), en_path)
        replace_local2remote(en_path)
        for i in ['_static', '_images', '_sources', '_downloads']:
            _rename_htmlconf(en_path, i)
        replace_en(en_path)
    else:
        raise FileExistsError("%s has exists." % cn_path)
